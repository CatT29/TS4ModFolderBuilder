...
# Set up logging with both file and console handlers, using a single log file
log_filepath = logs_directory / "mod_builder.log"
backup_log_filepath = logs_directory / "mod_builder.log.bak"

# Create a backup of the existing log file if it exists
if log_filepath.exists():
    try:
        ...
        # Create new backup
        shutil.copy2(log_filepath, backup_log_filepath)
    except Exception as e:
        print(f"Error creating log backup: {e}")

# Configure logging to use the single log file (overwriting it)
logging.basicConfig(
    level=logging.DEBUG,
...
class ModBuilderApp:
    ...
    def create_variables(self):
        ...
        self.package_var = tk.BooleanVar(value=False)  # Package now acts as "All"
        ...
        self.package_var.trace_add("write", self.on_package_toggle)  # New function
    ...
    def on_package_toggle(self, *args):
        """Ensure Package selection acts as 'All' """
    ...
    def generate_mod(self):
        ...
        try:
            ...
            if self.package_var.get() or self.script_var.get() or settings_data.get("init_py_in_scripts", True):
                create_init_py()
